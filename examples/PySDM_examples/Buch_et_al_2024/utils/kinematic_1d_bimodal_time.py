        ...
        *,
        ...
        with np.errstate(all="raise"):
            for i, (kappa, spectrum) in enumerate(aerosol_modes_by_kappa.items()):
                ...
                attributes["cell origin"] = np.array(
                    [np.append(attributes["cell origin"], cell_origin)]
                )
                attributes["position in cell"] = np.array(
                    [np.append(attributes["position in cell"], pos_cell)]
                )
                ...
                if collisions_only:
                    ...
                    attributes["multiplicity"] = np.append(
                        ...
                        n_per_kg * rhod[cell_id] * domain_volume,
                    )
    ...
    def inject_particles(
        ...
        *,
        ...
        with np.errstate(all="raise"):
            ...
            cell_id, cell_origin, pos_cell = self.mesh.cellular_attributes(
                np.array([positions[0, p_indx]])
            )
            self.particulator.attributes["cell id"].data = np.append(
                self.particulator.attributes["cell id"].data, cell_id
            )
            self.particulator.attributes["cell origin"].data = np.array(
                [
                    np.append(
                        self.particulator.attributes["cell origin"].data, cell_origin
                    )
                ]
            )
            self.particulator.attributes["position in cell"].data = np.array(
                [
                    np.append(
                        self.particulator.attributes["position in cell"].data, pos_cell
                    )
                ]
            )
            ...
            v_dry = self.formulae.trivia.volume(radius=r_dry)
            self.particulator.attributes["dry volume"].data = np.append(
                self.particulator.attributes["dry volume"].data, v_dry
            )
            self.particulator.attributes["kappa times dry volume"].data = np.append(
                self.particulator.attributes["kappa times dry volume"].data,
                v_dry * kappa,
            )
            self.particulator.attributes["multiplicity"].data = np.append(
                self.particulator.attributes["multiplicity"].data,
                np.ones(n_seed_sds) * m_param,
            )
            ...
            self.particulator.attributes["volume"].data = np.append(
                self.particulator.attributes["volume"].data,
                self.formulae.trivia.volume(radius=r_wet),
            )

            self.particulator.attributes["cell id"].data = np.array(
                self.particulator.attributes["cell id"].data, dtype=int
            )
            self.particulator.attributes["cell origin"].data = np.array(
                self.particulator.attributes["cell origin"].data, dtype=int
            )
            ...
            self.particulator.attributes["water mass"].data[-n_seed_sds:] = (
                self.particulator.formulae.particle_shape_and_density.volume_to_mass(
                    self.particulator.attributes["volume"].data[-n_seed_sds:]
                )
            )
